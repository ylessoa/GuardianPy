...
class FakeProcessNetwork:
    ...
    def connections(self, kind="inet"):
        return [FakeConnection(3333)]


class FakeProcessSustainedCPU:
    """Proceso simulado con CPU sostenida en varias iteraciones."""
    def __init__(self, pid=7890, name="sustained_miner", cpu_pattern=None):
        self.pid = pid
        self._name = name
        self.cpu_pattern = cpu_pattern or [85, 90, 92, 95]  # varias mediciones altas
        self.index = 0
        self.info = {
            "pid": pid,
            "name": name,
            "cpu_percent": self.cpu_pattern[0],
        }
    ...
    def cpu_percent(self, interval=None):
        # Simula CPU alta sostenida en varias llamadas
        value = self.cpu_pattern[self.index % len(self.cpu_pattern)]
        self.index += 1
        self.info["cpu_percent"] = value
        return value
...
def test_crypto_mining_detector_marks_sustained_cpu(monkeypatch, caplog):
    caplog.set_level(logging.CRITICAL)
    detector = CryptoMiningDetector(logger=logging.getLogger("GuardianPy"))

    import guardianpy.core.crypto_mining_detector as mining_detector
    fake_proc = FakeProcessSustainedCPU()
    monkeypatch.setattr(mining_detector.psutil, "process_iter", lambda attrs=None: [fake_proc])
